...
namespace miniplc0 {
	...
	std::optional<CompilationError> Analyser::analyseVariableDeclaration() {
        ...
        while (true) {
                ...
                next.value().GetType() != TokenType::SEMICOLON)
            {
                ...
                addUninitializedVariable(s.value());
                 //申请位置
                ...
                _instructions.emplace_back(Operation::STO, getIndex(std::any_cast<std::string>(s.value().GetValue())));
                //把结果村回去
	...
	std::optional<CompilationError> Analyser::analyseAssignmentStatement() {
        ...
        if (err.has_value())
            return err;
        _instructions.emplace_back(Operation::STO, getIndex(a.value().GetValueString()));
        addReUninitializedVariable(a.value());
        // '<表达式>'
        next = nextToken();
        if (!next.has_value() || next.value().GetType() != TokenType::SEMICOLON)
            return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNoSemicolon);
	...
	std::optional<CompilationError> Analyser::analyseFactor() {
		...
		switch (next.value().GetType()) {
		    case IDENTIFIER:
		        if(!isDeclared(next.value().GetValueString()))
                    return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNotDeclared);
		        if(isUninitializedVariable(next.value().GetValueString())&&!isReUnInitializedVariable(next.value().GetValueString())){
		    ...
		    case UNSIGNED_INTEGER:
                s=std::any_cast<std::string>(next.value().GetValue());
                lengths=s.length();
                if(lengths>=11)
                    return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrIntegerOverflow);
                for(int i=0;i<lengths;i++){
    ...
    void Analyser::addReUninitializedVariable(const Token& tk) {
        _add(tk, _reuninitialized_vars);
    }
    ...
    bool Analyser::isReUnInitializedVariable(const std::string &s) {
        return _reuninitialized_vars.find(s) != _reuninitialized_vars.end();
    }
